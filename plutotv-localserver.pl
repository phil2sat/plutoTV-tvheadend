            ...
            if(defined $logo) {
                ...
                if($directstreaming || $usestreamlink) {
                  ...
                  my $url = $sender->{stitched}->{urls}[0]->{url};
                    ...
                    $m3u .= "pipe://" . $ffmpeg . " -user-agent \"Mozilla/5.0 (X11; Ubuntu; Linux i686; rv:86.0) Gecko/20100101 Firefox/86.0\" -i \"http://" . $hostip . ":" . $port . "/master3u8?id=" . $sender->{_id} . "\" -map 0 -c copy -hide_banner -nostats -loglevel fatal -threads 2 -re -fflags +genpts+ignidx+igndts -f mpegts -mpegts_service_type advanced_codec_digital_hdtv -metadata service_name=\"" . $sender->{name} . "\" pipe:1\n";
