            ...
            if(defined $logo) {
                ...
                if($directstreaming || $usestreamlink) {
                  ...
                  my $url = $sender->{stitched}->{urls}[0]->{url};
                    ...
                    $m3u .= "pipe://" . $ffmpeg . " -threads 2 -re -i \"http://" . $hostip . ":" . $port . "/master3u8?id=" . $sender->{_id} . "\" -map 0 -c copy -hide_banner -nostats -loglevel fatal -fflags +genpts+ignidx+igndts -f mpegts -mpegts_service_type advanced_codec_digital_hdtv -metadata service_name=\"" . $sender->{name} . "\" -tune zerolatency pipe:1\n";
