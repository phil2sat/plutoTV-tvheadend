            ...
            if(defined $logo) {
                ...
                if($directstreaming || $usestreamlink) {
                  ...
                  my $url = $sender->{stitched}->{urls}[0]->{url};
                    ...
                    $m3u .= "pipe://" . $ffmpeg . " -loglevel fatal -threads 2 -re -i \"http://" . $hostip . ":" . $port . "/master3u8?id=" . $sender->{_id} . "\" -c copy -fflags +genpts+ignidx+igndts -f mpegts -tune zerolatency -mpegts_flags +initial_discontinuity -mpegts_service_type advanced_codec_digital_hdtv -metadata service_name=\"" . $sender->{name} . "\" pipe:1\n";
                }
            }
        }
    }
