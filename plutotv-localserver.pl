            ...
            if(defined $logo) {
                ...
                if($directstreaming || $usestreamlink) {
                  ...
                  my $url = $sender->{stitched}->{urls}[0]->{url};
                    ...
                    $m3u .= "pipe://" . $ffmpeg . " -map 0 -c copy -hide_banner -nostats -loglevel 0 -i \"http://" . $hostip . ":" . $port . "/master3u8?id=" . $sender->{_id} . "\" -c copy -f mpegts -mpegts_service_type advanced_codec_digital_hdtv -metadata service_name=\"" . $sender->{name} . "\" pipe:1\n";
                }
            }
        }
    }
